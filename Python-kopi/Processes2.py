...
class Processes2():
    def __init__(self, S0, r, T, K, paths, I, LNparams, JRparams, GBMparams):
        print ("Processes2 class created with S0 = %s, paths = %s, I = %s, T = %s" % (S0, paths, I, T))
        self.S0 = S0
        self.r = r
        self.T = T
        self.K = K
    ...
    def gbm_paths_sobol_antithetic_moment_matching_sample(self):
    ...
    def gbm_paths_sobol_antithetic_moment_matching(self, mean_normal, std_normal):
        mu, sigma = self.GBMparams
        S0, paths, I, T = self.S0, self.paths, self.I, self.T
        dt = T / I
    ...
    def merton_jump_paths_sobol_antithetic_moment_matching_sample(self):
    ...
    def merton_jump_paths_sobol_antithetic_moment_matching(self, mean_normal_LN, mean_lognormal_LN, std_normal_LN, std_lognormal_LN):
        lam, sigma, mu, v, m = self.LNparams
        S0, paths, I, T = self.S0, self.paths, self.I, self.T
        matrix = np.zeros((2 * paths, I))
        dt = T / I
    ...
    def merton_jump_to_ruin_paths_sobol_antithetic_moment_matching_sample(self):
    ...
    def merton_jump_to_ruin_paths_sobol_antithetic_moment_matching(self, mean_normal_JR, std_normal_JR):
        lam , sigma, mu = self.JRparams
        S0, paths, I, T = self.S0, self.paths, self.I, self.T
        matrix = np.zeros((2 * paths, I))
        dt = T / I
